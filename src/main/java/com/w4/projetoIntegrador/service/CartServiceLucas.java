...
public class CartServiceLucas {
        ...
        public CartServiceLucas(  CartRepository cartRepository,
                             BuyerService buyerService,
                             ItemCartService itemCartService,
                             ProductAnnouncementService productAnnouncementService){
        ...
        private void limitingQuantitySameProductCart(ItemCartDto itemCartDto){
            if(itemCartDto.getQuantity()>2){
                throw new BusinessException("não pode comprar um item mais de duas vezes.");
            }
        }
